...
class ApiCall {
  ...
  Future<ARoResult<ApiFailure, ApiSuccess>> request({
    ...
    try {
      ...
      // final responseBodyString = utf8.decode(petitionResponse.bodyBytes);
      // final decodedJson = jsonDecode(responseBodyString);

      // final apiResponse = ApiResponse<MyDataModel>(
      //   statusCode: petitionResponse.statusCode,
      //   headers: petitionResponse.headers,
      //   body: MyDataModel.fromJson(decodedJson),
      //   exception: null,
      //   isSuccess: petitionResponse.statusCode >= 200 && petitionResponse.statusCode < 300,
      // );
...
''', stackTrace: stackTrace);
  ...
  String getBaseUri(String env, String ep) {
    Map<String, String> map = {
      'dev': 'https://arosistemas.com$ep',
      'qa': 'https://arosistemas.com$ep',
      'prod': 'https://arosistemas.com$ep',
    };

    return map[env] ?? 'https://arosistemas.com$ep';
  }
}
